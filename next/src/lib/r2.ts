...
if (!R2_ACCOUNT_ID || !R2_ACCESS_KEY_ID || !R2_SECRET_ACCESS_KEY || !R2_BUCKET_NAME) {
  console.warn('R2 environment variables are not configured. File uploads will return metadata only.');
}

if (!R2_CUSTOM_DOMAIN) {
  console.warn('R2_CUSTOM_DOMAIN is not configured. File URLs will not be generated.');
}
